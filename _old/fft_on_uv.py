...
FILE_LOC = '/home/n02/n02/mmuetz/work/cylc-run/u-ax548/share/data/history/km1_large_dom_no_wind/atmosa_pc072.nc'
...
class FftProcUVW:
    ...
    def _calc_filtered(self,
        ...
        r = np.sqrt((x - HALF_NX)**2 + (y - HALF_NX)**2)
...
class Plotter:
    ...
    def _plot_pow_spectrum(self, power, height):
        ...
        r, r_pow_w2 = radial_profile(power, (HALF_NX, HALF_NX))

        # N.B. don't plot (HALF_NX, HALF_NX) which is zero freq term.
        # Single row along x
        plt.loglog(range(1, HALF_NX), power[HALF_NX, HALF_NX + 1:], 'b-', label='x-dir')
        # Single col along y
        plt.loglog(range(1, HALF_NX), power[HALF_NX + 1:, HALF_NX], 'r-', label='y-dir')
        # Radial sum.
        plt.loglog(r[1:HALF_NX + 1], r_pow_w2[1:HALF_NX + 1], 'g-', label='radial')

        x = np.linspace(4, HALF_NX, 2)
        y = 215443469 * x ** (-5 / 3)
